...
export async function fetchLauncherToken(username: string, password: string): Promise<OAuthTokenResponse> {
...
export async function fetchExchangeToken(accessToken: string): Promise<OAuthExchangeResponse> {
...
export async function fetchFortniteToken(exchangeCode: string): Promise<OAuthTokenResponse> {
...
export async function fetchRefreshToken(refreshToken: string): Promise<OAuthTokenResponse> {
  const headers = { authorization: `basic ${FORTNITE_AUTHORIZATION}` };
